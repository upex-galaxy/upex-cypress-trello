Feature: Update Cards Cover
    ...
    Scenario Outline: 1086 | TC1: Validar que el usuario con acceso a API, agrega un cover para la card
        When usuario envía el request de 'Update a Card' con el siguiente parámetro: <datos> en <parámetro 1> <parámetro 2>
        Then automáticamente '<resultado>' agrega el cover a la card
        Examples:
            | datos                                     | parámetro 1  | parámetro 2  | resultado    |
            | pink                                      | color        |              | se           |
            | yellow                                    | color        |              | se           |
            | lime                                      | color        |              | se           |
            | black                                     | color        |              | se           |
            ...
            | sky                                       | color        |              | se           |
            | green                                     | color        |              | se           |
            | que contiene números                      | color        |              | se           |
            | que está fuera de la documentación        | color        |              | se           |
            | dark                                      | brightness   |              | se           |
            | light                                     | brightness   |              | se           |
            ...
            | que url que no sea de Unsplash            | url          |              | no se        |
            ...
            | full                                      | size         |              | se           |
            | null                                      | size         |              | se           |
            | null                                      | url          | idAttachment | el color se  |
            | null                                      | color        | idAttachment | la imagen se |
            | null                                      | color        | url          | la imagen se |
